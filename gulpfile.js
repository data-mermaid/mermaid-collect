...
gulp.task('build_info', function() {
  var version = execSync('git rev-parse --verify HEAD --short').toString().trim();
  return string_src('app_version.js', 'var MERMAID_VERSION="local-' + version + '";')
...
gulp.task('lint', function() {
  return gulp.src('./src/build/app.js')
    .pipe(jshint())
...
gulp.task('generate-sw', function(callback) {
  ...
  swPrecache.write(path.join(rootDir, 'sw.js'), {
    staticFileGlobs: [rootDir + '/**/*.{js,html,css,png,jpg,gif,svg,eot,ttf,woff,woff2,json,pdf}'],
    stripPrefix: rootDir,
...
gulp.task('local', ['build_info', 'vendor', 'js', 'generate-sw', 'dev-lint', 'watch', 'connect']);
