  ...
  .directive('speechRecognition', function($log) {
    ...
    $.speechApp = (function(speech) {
      ...
      speech.playON = function() {
        ...
        if (navigator.userAgent.match('Firefox/'))
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_on' + '.ogg'
          );
        else
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_on' + '.mp3'
          );
      ...
      speech.playOFF = function() {
        ...
        if (navigator.userAgent.match('Firefox/'))
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_off' + '.ogg'
          );
        else
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_off' + '.mp3'
          );
      ...
      speech.playConfirmation = function() {
        ...
        if (navigator.userAgent.match('Firefox/'))
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_alert' + '.ogg'
          );
        else
          audioElement.setAttribute(
            'src',
            appConfig.sound_path + 'voice_alert' + '.mp3'
          );
    ...
    (function(undefined) {
      ...
      var commandsList = [],
        ...
        splatParam = /\*\w+/g,
        escapeRegExp = /[\-{}\[\]+?.,\\\^$|#]/g,
        commandToRegExp = function(command) {
          command = command
      ...
      root.smartSpeechRecognition = {
        ...
        init: function(commands, resetCommands) {
          ...
          recognition.onstart = function() {
            ...
            if (appConfig.debugState) {
              root.console.log(
                '%c ✔ SUCCESS: User allowed access the microphone service to start ',
                appConfig.debugStyle_success
              );
              root.console.log(
          ...
          recognition.onresult = function(event) {
            ...
            for (var i = 0; i < results.length; i++) {
              ...
              if (appConfig.debugState) {
                root.console.log(
                  'Speech recognized: %c' + commandText,
                  appConfig.debugStyle
                );
              }
              ...
              for (var j = 0, l = commandsList.length; j < l; j++) {
                ...
                if (result) {
                  ...
                  if (appConfig.debugState) {
                    root.console.log(
                      'command matched: %c' + commandsList[j].originalPhrase,
                      appConfig.debugStyle
                    );
                  ...
                  if (
                    ignoreCallsFor.indexOf(commandsList[j].originalPhrase) < 0
                  ) {
        ...
        addCommands: function(commands) {
          ...
          if (appConfig.debugState) {
            root.console.log(
              'Commands successfully loaded: %c' + commandsList.length,
              appConfig.debugStyle
            );
          }
        },
    ...
    if (
      ...
      localStorage.getItem('sm-setautovoice') == 'true'
    ) {
      autoStart();
    }
    ...
    var link = function(scope, element) {
      if (SpeechRecognition && appConfig.voice_command) {
        ...
        var modal = $(
          '<div class="modal fade" id="voiceModal" tabindex="-1" role="dialog" aria-labelledby="remoteModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"></div></div></div>'
        );
        // attach to body
        modal.appendTo('body');
        ...
        $(document).mouseup(function(e) {
          if (
            !$('#speech-btn .popover').is(e.target) &&
            $('#speech-btn .popover').has(e.target).length === 0
          ) {
            $('#speech-btn .popover').fadeOut(250);
          }
        });
