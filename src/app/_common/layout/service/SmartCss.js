  ...
  .factory('SmartCss', function($rootScope, $timeout) {
    ...
    var SmartCss = {
      writeRule: function(selector) {
        ...
        if (_.has(_styles, selector)) {
          var css =
            selector +
            '{ ' +
            _.map(_styles[selector], function(v, k) {
              return k + ':' + v + ';';
            }).join(' ') +
            '}';
      ...
      add: function(selector, property, value, delay) {
        if (!_.has(_styles, selector)) _styles[selector] = {};

        if (value == undefined || value == null || value == '')
          delete _styles[selector][property];
        else _styles[selector][property] = value;

        if (_.keys(_styles[selector]).length == 0) delete _styles[selector];
